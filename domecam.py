...
class DCSettings(bpy.types.PropertyGroup):
    ...
    resolution = IntProperty(name='Resolution', description='Resolution of finished Domemaster', default=1024, min=128, max=10000, update=onUpdateResolution)
    ...
    file_format = EnumProperty(items=image_format, name='File format', description='', default='png')
...
#   use_domecam = BoolProperty(name='Use Domecam', description='Use Domecam System for Render', default=False)
    ...
    use_comp = BoolProperty(name='Use compositor on Domemaster', description='Use Postprocessing on Domemaster', default=False)
...
class OpAddMirrorCam(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        cam_ob.location = 0.0, 0.0, 0.200
        cam_ob.scale = 0.100, -0.100, 0.100
        ...
        #make mirror
        '''
            Fisheye Camera Rig Developed by
            Ron Proctor - Ott Planetarium
            
        License Details and Full Text:
        ...
        mirror.hide_select = True
        
        #mirror material
        mirror_mat = bpy.data.materials.new(name="MirrorCam")
        mirror_mat.diffuse_color = [1.000000, 1.000000, 1.000000]
        mirror_mat.use_shadeless = True
        mirror_mat.raytrace_mirror.use = True
        mirror_mat.raytrace_mirror.reflect_factor = 1.000
        mirror_mat.mirror_color = [1.000000, 1.000000, 1.000000]
        mirror_mat.raytrace_mirror.fresnel = 0.000
        mirror_mat.raytrace_mirror.depth = 64
        mirror_mat.raytrace_mirror.distance = 10000.000
        mirror_mat.use_full_oversampling = True 
        mirror_mat.use_raytrace = False
        mirror_mat.use_mist = False
        mirror_mat.use_shadows = False
        mirror_mat.use_cast_buffer_shadows = False
        mirror_mat.use_ray_shadow_bias = False
        mirror_mat.use_cast_approximate = False
        
        mirror.data.materials.append(mirror_mat)
        
        #make plane
        verts = [(0.5, 0.5, 0.0), (0.5, -0.5, 0.0), (-0.5, -0.5, 0.0), (-0.5, 0.5, 0.0), (0.0, 0.9, 0.0), (-0.5, 0.5, -0.0561), (0.0, 0.9, -0.0561), (0.5, 0.5, -0.0561), (-0.5, -0.5, -0.0561), (0.5, -0.5, -0.0561)]
        faces = [[4, 2, 1], [0, 4, 1], [4, 3, 2], [3, 4, 6, 5], [4, 0, 7, 6], [2, 3, 5, 8], [0, 1, 9, 7], [1, 2, 8, 9], [6, 8, 5], [7, 9, 6], [6, 9, 8]]
        ...
        plane = bpy.data.objects.new("MirrorCam",plane_mesh)
        plane.domecam_type = "MirrorCam"
        plane.lock_scale = True, True, True
        scene.objects.link(plane)
        
        #plane material
        plane_mat = bpy.data.materials.new(name="MirrorCam_plane")
        plane_mat.diffuse_color = [0.000000, 0.000000, 0.000000]
        plane_mat.diffuse_intensity = 0
        plane_mat.specular_color = [0.000000, 0.000000, 0.000000]
        plane_mat.specular_intensity = 0
        plane_mat.ambient = 0
        plane_mat.use_raytrace = False
        plane_mat.use_mist = False
        plane_mat.use_shadows = False
        plane_mat.use_cast_buffer_shadows = False
        plane_mat.use_ray_shadow_bias = False
        plane_mat.use_cast_approximate = False
        ...
        bpy.ops.dc.updatelist()
        scene.dc_render.cur_mirror_cam = plane.name
...
class OpRenderDomemaster(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        sce.render.use_sequencer = False
        ...
        for cam in sce.objects[dc.cur_multi_cam].children:
            ...
            if (dc.use_border and cam.domecam_view != 'top'):
                ...
                sce.render.use_crop_to_border = False
        ...
        if (dc.use_comp):
            ...
            if ('domecam_composite_image' in bpy.data.images):
                image = bpy.data.images['domecam_composite_image']
                #image.user_clear()
                image.filepath=os.path.join(renderpath,'dm',filename+'.png')
                image.reload()
            else:
                image = bpy.data.images.load(filepath=os.path.join(renderpath,'dm',filename+'.png'))
                image.name = 'domecam_composite_image'
